...
`[截图位 12：Keil 中三个工程的目录结构]`

### 8.14 实际 OTA 升级流程

完整的 OTA 升级流程如下：

1. 设备上电，运行 app_ota（假设当前在 App1）
2. 手机通过 nRF Connect 或专用 App 连接设备
3. 手机发送升级指令，触发设备重启到 ImageUpdate
4. masterboot 检测到 ImageUpdate 激活，跳转到 ImageUpdate
5. ImageUpdate 开始广播，手机重新连接
6. 手机通过 NS_IUS 服务发送新固件
7. ImageUpdate 将新固件写入 App2 分区
8. CRC 校验通过，更新 Bootsetting
9. 重启，masterboot 跳转到 App2
10. 设备以 App2 身份广播 `N32WB031_OTA_A2`

![alt text](66c336f3ab761a3f8753d2d956002480.jpeg)
`[截图位 14：OTA 升级进度界面]`
`[截图位 15：升级完成，设备广播名变为 N32WB031_OTA_A2]`
...
`[截图位 16：NSAndroidUtilV1.5.apk 文件位置截图]`
