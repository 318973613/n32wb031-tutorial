...
void app_ble_msg_handler(struct ble_msg_t const *p_ble_msg)
{
    switch (p_ble_msg->msg_id)
    {
        ...
        case APP_BLE_GAP_CONNECTED:
            app_rdtsc_16bit_enable_prf(app_env.conidx);
            app_ble_connected();
            break;
...
//    static const uint8_t target_addr[] = {"\x11\x11\x11\x11\x11\x11"};
    static const uint8_t target_name[] = {"NS_RDTSS_16"};
...
{
    //add 16bit raw data transmit server(rdts)
    ns_ble_add_prf_func_register(app_rdtsc_16bit_add_rdts);
}
