...
{
 struct gap_bdaddr peer_addr = {0};   
    switch (msgid)
    {
    	case APP_RECONNECT_EVT:		
					ns_bond_last_bonded_addr(&peer_addr);
					NS_LOG_DEBUG("\r\nAddr: ");
					for(int i = 0; i < 6; i++)
					{
						NS_LOG_DEBUG("%02x ", peer_addr.addr.addr[i]);
					}
					NS_LOG_DEBUG("\r\naddr type: %x, ", peer_addr.addr_type);
					ns_ble_list_set_wl(&peer_addr,1);
					ns_ble_start_init(NULL, peer_addr.addr_type);				
    		break;	
        ...
        case APP_BLE_OS_READY:
            NS_LOG_INFO("APP_BLE_OS_READY\r\n");
				   if (ns_sec_get_bond_status())
					 {
							ke_timer_set(APP_RECONNECT_EVT, TASK_APP, 500);								
					 }				
            break;
        case APP_BLE_GAP_CONNECTED:
						if (ns_sec_get_bond_status())
						{
							ns_sec_send_encrypt_req(app_env.conidx);
						}
						else
						{
							ns_sec_send_bond_start(app_env.conidx);
						}					
				#if (CFG_PRF_HOGPRH)
...
//    static const uint8_t target_addr[] = {"\x11\x11\x11\x11\x11\x11"};
    static const uint8_t target_name[] = {"hid_mouse"};
//    static const uint16_t target_uuid16 = 0x180A;
    ...
    init.initiating_timeout = 0;//3000;
...
{
	#if (CFG_PRF_HOGPRH)
	ns_ble_add_prf_func_register(app_hogprh_add_hid);
	#endif
}
...
void app_ble_disconnected(void)
{
		...
		else if (app_env.wl_reconnect) 
	  {
		 ke_timer_set(APP_RECONNECT_EVT, TASK_APP, 100);
	  }
